...
function makePieceMap(req) {
  ...
  const pieceNameArray = convertReqCookieIntoArray(req.cookies[config.KEY_PIECES]); // => 例[ABC, DEF, GHI...] 曲ID(n0, n1, n2...) に1対1で対応
  ...
  pieceNameArray.forEach((pieceName, pieceNameIdx) => {
    ...
    function makePlayerCountMap(pieceNameIdx) {
      ...
      playerCountArray.filter((v, i) => i >= peaceStartIdx && i <= peaceStartIdx + memberSum - 1).forEach((v, i) => {
...
module.exports = { convertReqCookieIntoArray, makePieceMap, assignParts, formatReqCookie };